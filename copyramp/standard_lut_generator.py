...
def main():
    ...
    parser.add_argument("filename_lut",help="insert the lut filename" )    
    parser.add_argument("f0_ch1", help="start frequency channel 1 [MHz]", type=float)
    parser.add_argument("fn_ch1", help="stop frequency channel 1 [MHz]", type=float)
    parser.add_argument("f0_ch2", help="start frequency channel 2 [MHz]", type=float)
    parser.add_argument("fn_ch2", help="stop frequency channel 2 [MHz]", type=float)
...
def create_xml_lut(args): 
    ...
    filename = args.filename_lut + '.xml'
    fp = open(filename ,'w')
